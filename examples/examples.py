...
def get_entity_types_by_id(pdb_id: str)  -> Dict[str, str]:
    ...
    type_by_id = dict(zip(entities.getAttributeValueList('id'), entities.getAttributeValueList('type')))
    return type_by_id
...
def default_story(pdb_ids: List[str], titles: List[str] | None = None, descriptions_md: List[str] | None = None, global_description: str | None = None) -> str:
    """
    ...
    snapshots = [default_snapshot(pdb_id, title, description) for pdb_id, title, description in zip(pdb_ids, titles, descriptions_md)]
    states_json = mvs.States(snapshots=snapshots, 
                        metadata=mvs.GlobalMetadata(description=global_description)
                        ).dumps(indent=2)
    return states_json
...
def default_snapshot(pdb_id: str, title: str | None = None, 
